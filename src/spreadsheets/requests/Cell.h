...
namespace GSHEET
{
     ...
     */
    class DataValidationRule : public O6
    {

    public:
        DataValidationRule() {}
        // The condition that data in the cell must match.
        DataValidationRule &condition(const BooleanCondition value) { return wr.set<DataValidationRule &, BooleanCondition>(*this, value, buf, bufSize, buf[1], FPSTR(__func__)); }
        // A message to show the user when adding data to the cell.
        DataValidationRule &inputMessage(const String &value) { return wr.set<DataValidationRule &, String>(*this, value, buf, bufSize, buf[2], FPSTR(__func__)); }
        // True if invalid data should be rejected.
        DataValidationRule &strict(bool value) { return wr.set<DataValidationRule &, bool>(*this, value, buf, bufSize, buf[3], FPSTR(__func__)); }
        // True if the UI should be customized based on the kind of condition. If true, "List" conditions will show a dropdown.
        DataValidationRule &showCustomUi(bool value) { return wr.set<DataValidationRule &, bool>(*this, value, buf, bufSize, buf[4], FPSTR(__func__)); }
    };

    /**
     * Data about a specific cell.
     */
    class CellData : public O12
    {

    public:
        CellData() {}
        // The value the user entered in the cell. e.g., 1234, 'Hello', or =NOW() Note: Dates, Times and DateTimes are represented as doubles in serial number format.
        CellData &userEnteredValue(const ExtendedValue &value) { return wr.set<CellData &, ExtendedValue>(*this, value, buf, bufSize, buf[1], FPSTR(__func__)); }
        // The formatted value of the cell. This is the value as it's shown to the user. This field is read-only.
        CellData &formattedValue(const String &value) { return wr.set<CellData &, String>(*this, value, buf, bufSize, buf[2], FPSTR(__func__)); }
        // The format the user entered for the cell.
        // When writing, the new format will be merged with the existing format.
        CellData &userEnteredFormat(const CellFormat &value) { return wr.set<CellData &, CellFormat>(*this, value, buf, bufSize, buf[3], FPSTR(__func__)); }
        ...
        CellData &pivotTable(const PivotTable &value) { return wr.set<CellData &, PivotTable>(*this, value, buf, bufSize, buf[8], FPSTR(__func__)); }
        ...
        CellData &dataSourceTable(const DataSourceTable &value) { return wr.set<CellData &, DataSourceTable>(*this, value, buf, bufSize, buf[9], FPSTR(__func__)); }
    };
}
#endif